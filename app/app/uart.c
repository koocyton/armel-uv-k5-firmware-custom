...
static void CMD_05F0(uint32_t Port, const uint8_t *pBuffer)
{
...
static void CMD_05F2(uint32_t Port, const uint8_t *pBuffer)
{
...
#if defined(ENABLE_SI4732) && (defined(ENABLE_UART) || defined(ENABLE_USB))
        case 0x05F0:
            CMD_05F0(Port, pUART_Command->Buffer);
            break;

        case 0x05F2:
            CMD_05F2(Port, pUART_Command->Buffer);
            break;
#endif
