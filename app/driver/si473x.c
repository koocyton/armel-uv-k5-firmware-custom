...
static uint32_t Read_FreqSaved(void)
{
    ...
    uint32_t tmpF;
    I2C_EEPROM_ReadBuffer(SI473X_EEPROM_FREQ_BASE, (uint8_t *)&tmpF, 4);
    if (!FreqCheck(tmpF))
        tmpF = 10210000;
    return tmpF;
}
...
static bool SI47XX_downloadPatch(void) {
  ...
  for (uint16_t offset = 0; offset < SI473X_PATCH_SIZE; offset += sizeof(buf)) {
    ...
    I2C_EEPROM_ReadBuffer32(SI473X_PATCH_EEPROM + offset, buf, (uint16_t)n);
...
static void SI47XX_PatchPowerUp(void) {
  ...
  {
    uint8_t bfoBuf[4];
    I2C_EEPROM_ReadBuffer(SI473X_EEPROM_AM_KHZ, bfoBuf, 4);
